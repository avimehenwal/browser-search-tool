...
export function registerDexieIndexSync() {
  ...
  try {
    ...
    (db.photoMetadata as any).hook("created", (primKey: any, obj: any) => {
      try {
    ...
    (db.photoMetadata as any).hook(
      "updating",
      (modifications: any, primKey: any, obj: any) => {
        try {
          const id = Number(primKey);
    ...
    (db.photoMetadata as any).hook("deleting", (primKey: any) => {
      try {
        const idx = (flexIndex as any).getIndex?.();
        const id = Number(primKey);
