...
function DropdownMenuTrigger({
  ...
  // Strip `asChild` to avoid leaking it to native DOM elements.
  // If a single React element child is provided (e.g., a <Button />), forward it
  // as the `render` prop so the primitive will clone it and avoid nesting
  // a <button> inside another <button>.
  if (React.isValidElement(children) && !renderProp) {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore - MenuPrimitive.Trigger accepts `render` at runtime
    return (
  ...
  // If a `render` prop was provided, forward it to the primitive.
  if (renderProp) {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore - MenuPrimitive.Trigger accepts `render` at runtime
    return (
  ...
  // Otherwise render with children (if any) passed through.
  return (
...
function DropdownMenuItem({
  ...
  // Support callers passing an `onSelect` handler (used throughout the
  // codebase). Some underlying primitives may not call `onSelect`, so also
  // invoke it from an `onClick` handler to make selection reliable.
  const { onClick: originalOnClick, onSelect, ...restProps } = props as any;
  const handleClick = (ev: any) => {
    try {
      if (typeof onSelect === "function") onSelect(ev);
    } catch {}
    try {
